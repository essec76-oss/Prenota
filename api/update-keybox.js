...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // ------------------------------------------------------------
    // 3) Leggi il nuovo codice dal corpo della richiesta
    // ------------------------------------------------------------
    const { codice_keybox } = req.body || {};

    if (!codice_keybox || typeof codice_keybox !== 'string' || !codice_keybox.trim()) {
      return res.status(400).json({ error: 'Il codice key box non può essere vuoto.' });
    }
    ...
    const updateRes = await fetch(SUPABASE_URL + '/rest/v1/Impostazioni?id=eq.1', {
      ...
      body: JSON.stringify({
        codice_keybox: codice_keybox.trim(),
    ...
    return res.status(200).json({ success: true, codice_keybox: codice_keybox.trim() });
